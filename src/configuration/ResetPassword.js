...
export default function ResetPassword() {
  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        // Valider le token
        const response = await axios.get(`http://localhost:8081/mutuelle/member/validate-reset-token?token=${token}`);
        if (response.status === 200) {
          setTokenValid(true);
          
          // Récupérer les informations du membre
          const memberResponse = await axios.get(`http://localhost:8081/mutuelle/member/member-by-token?token=${token}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8081/mutuelle/member/reset-password', {
        token: token,
