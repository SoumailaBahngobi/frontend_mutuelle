...
export default function LoanValidationReports() {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8081/mutuelle/member/profile', {
  ...
  const fetchReports = async () => {
    try {
      ...
      const statsRes = await axios.get('http://localhost:8081/mutuelle/loan-validator/dashboard', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setStats(statsRes.data);

      // Récupérer l'historique des validations
      const historyRes = await axios.get('http://localhost:8081/mutuelle/loan-validator/my-approval-history', {
        headers: { Authorization: `Bearer ${token}` }
      });
